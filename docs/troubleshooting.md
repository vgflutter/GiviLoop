...
Real anonymous ChatGPT MCP reviews passed on macOS, Windows Server 2025 and Ubuntu X11, with no visible browser windows or foreground activation sampled by calibrated native observers. Authenticated windowless checks, Windows 11/ARM and Wayland remain unvalidated. The earlier minimized transport could flash during input on some desktops; those historical results do not describe the current hidden-target implementation. [Detailed evidence and requirements](windowless-browser.md).
