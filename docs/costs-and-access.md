...
**Optional browser automation:** GiviLoop fills, sends and reads a supported chat page automatically, then saves the response. [Validation differs by provider](web-providers.md). `--background` uses a hidden page in ordinary Chrome with a bundled offscreen extension. [Windowless validation](windowless-browser.md) is separate from earlier minimized-browser results. Login and human verification still require the account holder when requested. The integration has completed real authenticated CLI/MCP reviews, but the website can change or refuse access.
