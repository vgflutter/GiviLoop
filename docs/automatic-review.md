...
3. GiviLoop sends the selected diff and current source to the reviewer pinned at enable time. No provider fallback occurs. Ordinary Chrome runs the review in a hidden page, supported by the bundled offscreen extension. [Windowless validation](windowless-browser.md).
