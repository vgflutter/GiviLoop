# A second check before your commit
...
## Small adoption pilot

Invite five developers who already use a coding agent to try the demo, then one real change. Ask for a second use only if they found the first useful. The pilot has not yet been run; no recruitment messages or community posts have been sent.

Collect only voluntary public/synthetic examples through the [Double Check experience form](https://github.com/vgflutter/GiviLoop/issues/new?template=double_check.yml):

- Did installation and the first review complete? Where did they stop?
- Did an independently verified finding change their decision? Were there false positives?
- How much active setup/verification time did it take?
- Did they actually use it on a second change? Why or why not?

Count completed first reviews and actual repeat use out of the five invited users. Record failures as well as successes. No telemetry or private-source collection is required. Use a real, permissioned case with reproducible evidence for outreach; the bundled demo alone is not evidence of production value.
