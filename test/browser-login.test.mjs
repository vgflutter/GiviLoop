...
function login(f, executable, profile) {
  return spawnSync(process.execPath, [cliPath, "browser", "login", "--browser-profile", profile], {
...
test("native login requests a new visible window and the browser survives the CLI", { skip: process.platform === "win32" }, t => {
  ...
  const result = login(f, executable, profile);
